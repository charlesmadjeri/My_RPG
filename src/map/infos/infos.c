...
static void get_positions(sfView *view, infos *infos)
{
    ...
    // infos->spell_bar->pos.x = (sfView_getCenter(view).x + 250);
    // infos->spell_bar->pos.y = (sfView_getCenter(view).y - 570);
}

void display_infos(sfRenderWindow *window, game *game)
{
    get_positions(game->view, game->map->infos);
    // display_spell_bar(window, game->battle, game->map->infos->spell_bar);
